...
public class GenerateBowlerProfile {
	...
	public static void main(String args[]) {
		...
		ArrayList<String> allBowlersList = getAllBowlersInInnings(session,1);
		/*********************************************************/
	...
	public static ArrayList<String> getAllBowlersInInnings(Session session, int inningsId) {
		session.beginTransaction();

		Criteria criteria = session.createCriteria(Bowl.class).setFetchMode("Innings_Bowl", FetchMode.JOIN);
		criteria.setProjection(
				Projections.distinct(Projections.property("bowler"))
				).add(Restrictions.eq("InningsId", inningsId));
	...
	public static ArrayList<Bowl> getBowlersDeliveriesList(Session session,
		...
		"'" + bowler + "'" + "&& Bowl.bowlId in (SELECT deliveries_bowlId from Innings_Bowl where Innings_InningsId='1')");
		ArrayList<Bowl> bowlList = (ArrayList<Bowl>) query.list();
		System.out.println(bowlList.toString());
		session.getTransaction().commit();
		return bowlList;
	}
}
